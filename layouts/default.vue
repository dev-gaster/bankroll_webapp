<template>
  <v-app>
    ...
    <v-navigation-drawer v-model="drawer" class="d-flex d-sm-none" width="400" temporary>
      <template #prepend>
        <v-toolbar flat color="transparent" class="d-flex align-center justify-space-between">
          <!-- logo -->
          <NuxtLink to="/">
      ...
      <v-list nav>
        <v-list-item v-for="(link, index) in links" :key="index" :to="link.to" link>
    ...
    <v-app-bar color="transparent" flat :scroll-behavior="route.path.includes('campaigns') ? 'hide' : 'none'"
      ...
      <v-btn height="40" class="font-weight-medium text-white" prepend-icon="mdi-arrow-down" style="font-size: 16px"
        color="primary" variant="flat" depressed target="_blank"
        href="https://play.google.com/store/apps/details?id=com.gasobu.fintech_apps.bankroll&pcampaignid=web_share">
        Get the App
      </v-btn>
    </v-app-bar>
